...
public class main {
	...
	public static void main(String[] args) {
		...
		Map<Integer, Set<Venditore>> e = vends.entrySet().stream().
				collect(
						groupingBy(t -> t.getValue(),
								mapping(Map.Entry<Venditore, Integer>::getKey,
										toCollection(() -> new TreeSet<Venditore>()))));
		e.entrySet().stream().forEach(t -> System.out.println(t.getKey() + " = " + t.getValue().stream().map(x -> x.getNome()+":"+x.getEta()).collect(joining(", "))));
		
		Map<Integer,Map<Integer,List<String>>> jjj = vends.entrySet().stream().collect(
				groupingBy(t -> t.getValue(),
							groupingBy(x -> x.getKey().getEta(),
									mapping(x -> x.getKey().getNome(), toList()))
								));
